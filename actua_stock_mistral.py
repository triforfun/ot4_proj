...
Autor: mistral ai (basado en tu código inicial)
Fecha: 2025
"""
...
class StockUpdater:
    def __init__(self, carpeta_entrada, carpeta_salida, carpeta_backup):
        ...
        self.carpeta_orca = Path("C:/TFF/DOCS/ONLINE/STOCKS_EXTERNS/ORCA")
        self.carpeta_pujaftp = Path("C:/pujaftp")

        # Crear carpetas si no existen
        self.carpeta_salida.mkdir(parents=True, exist_ok=True)
        self.carpeta_backup.mkdir(parents=True, exist_ok=True)
        self.carpeta_pujaftp.mkdir(parents=True, exist_ok=True)
    ...
    def leer_archivo_proveedor(self, archivo_nombre, config):
        ...
        col_ean_original = campos_mapeo.get('ean')
        col_stock_original = campos_mapeo.get('stock')
        col_indexbusca_original = campos_mapeo.get('indexbusca')

        if archivo_nombre == 'STOCK_ORCA.CSV':
            # Para STOCK_ORCA.CSV, necesitamos crear un índice y luego mapear con orca_creuat.csv
            df_resultado = df.copy()
            df_resultado['indexbusca'] = df['Artículo'] + df['Descripción'] + df['Talla'] + df['Código Color'] + df['Color']
            df_resultado['stock'] = df['Unidades disponibles'].apply(self.normalizar_stock)
            df_resultado = df_resultado[['indexbusca', 'stock']]

        elif archivo_nombre == 'orca_creuat.csv':
            # Para orca_creuat.csv, simplemente leemos el mapeo de indexbusca a ean
            df_resultado = df[[col_indexbusca_original, col_ean_original]].copy()
            df_resultado.columns = ['indexbusca', 'ean']
        ...
        df_resultado.dropna(subset=['ean'] if 'ean' in df_resultado.columns else ['indexbusca'], inplace=True)

        if 'stock' in df_resultado.columns:
    ...
    def procesar_actualizacion(self):
        ...
        if local_config:
            ...
            if df_local is not None:
                ...
                proveedores_info['local'] = {'actualizados': len(eans_en_local), 'sin_match': len(df_principal) - len(eans_en_local)}
            else:
                archivos_con_errores.append('extract_produits_tailles.csv')

        # Leer el archivo orca_creuat.csv para obtener el mapeo de indexbusca a ean
        orca_creuat_config = self.proveedores_config.get('orca_creuat.csv')
        if orca_creuat_config:
            self.logger.info("Procesando archivo de mapeo ORCA (orca_creuat.csv)...")
            df_orca_creuat = self.leer_archivo_proveedor('orca_creuat.csv', orca_creuat_config)
            if df_orca_creuat is not None:
                orca_mapeo = dict(zip(df_orca_creuat['indexbusca'], df_orca_creuat['ean']))
            else:
                archivos_con_errores.append('orca_creuat.csv')
        ...
        def procesar_proveedor(archivo_nombre, config):
            ...
            actualizados = 0
            if archivo_nombre == 'STOCK_ORCA.CSV':
                # Para STOCK_ORCA.CSV, necesitamos mapear los indexbusca a ean usando orca_creuat.csv
                for indexbusca, stock in df_proveedor[['indexbusca', 'stock']].values:
                    ean = orca_mapeo.get(indexbusca)
                    if ean:
                        idx = ean_to_idx.get(ean)
                        if idx is not None:
            ...
            if proveedor not in proveedores_info:
                proveedores_info[proveedor] = {'actualizados': 0, 'sin_match': 0}
            proveedores_info[proveedor]['actualizados'] += actualizados
            proveedores_info[proveedor]['sin_match'] += len(df_proveedor) - actualizados
        ...
        # Copiar el archivo resultante a C:/pujaftp con el nombre nou_tff_auto.csv
        archivo_pujaftp_path = self.carpeta_pujaftp / "nou_tff_auto.csv"
        try:
            shutil.copy2(archivo_salida_path, archivo_pujaftp_path)
            self.logger.info(f"Copia del archivo de salida generada exitosamente en: {archivo_pujaftp_path}")
        except Exception as e:
            self.logger.error(f"Error al copiar el archivo de salida a '{archivo_pujaftp_path}': {e}")
